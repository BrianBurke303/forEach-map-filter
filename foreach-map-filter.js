...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(words){
      ...
      let last = words.charAt(words.length -1)
      let both = first.concat(last)
      firstLast.push(both)
    })
    return firstLast
}
...
function extractKey(arr, key){
    return arr.map(function(keyVal){
        return keyVal.name
    })
}
...
function findInObj(arr, key, searchValue) {
    return arr.filter(function(callback){
        return callback[key] === searchValue
    }).shift()
}
...
function doubleOddNumbers(arr) {
    let oddNums = []
    // let oddNumsDoubled = []
    arr.filter(function(odds){
        if (odds % 2 !== 0){
            oddNums.push(odds * 2)
        }
    })
    // return oddNums.map(function(nums){
    //     return oddNums * 2;
    // })
    return oddNums
}
